...
def fetchinfo():
    ...
    while(new == None ):
        try:
            ...
            new = requests.get('https://sportsworld165.com/collections/new-arrivals/fitted.atom',proxies=proxy_dict[choice], timeout=(3, 6))
        ...
        # catastrophic error. bail.
            send_Error("Error occured!", "Proxy failed "+str(proxy_dict[choice]))
            choice = random.randint(0,999)
            print("Retrying")
...
def main():
    ...
    while(True):
        ...
        if(int(current) >= 1559 and int(current) <= 1602):
            awake = True
